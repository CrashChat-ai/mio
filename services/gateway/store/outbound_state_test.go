...
func TestOutboundState_SetGet(t *testing.T) {
	...
	got, ok := s.Get(t.Context(), "cmd-1", "acct-test")
	if !ok {
...
func TestOutboundState_GetMissing(t *testing.T) {
	s := NewOutboundState()
	_, ok := s.Get(t.Context(), "nonexistent", "acct-test")
	if ok {
...
func TestOutboundState_TTLExpiry(t *testing.T) {
	...
	_, ok := s.Get(t.Context(), "cmd-ttl", "acct-test")
	if ok {
...
func TestOutboundState_LRUEviction(t *testing.T) {
	...
	_, ok := s.Get(t.Context(), "a", "acct-test")
	if ok {
		t.Fatal("expected 'a' to be evicted as LRU")
	}
	// "d" must be present.
	got, ok := s.Get(t.Context(), "d", "acct-test")
	if !ok || got != "ext-d" {
