...
the variables below; everything else lives in `gateway/internal/config`.
...
| `MIO_AGE_KEY_FILE` | _empty_ | Path to an age identity file. Read by `gateway/internal/crypto.NewAgeFileCipher` to encrypt credentials at rest. In `dev` an empty value falls back to `NoopCipher`; outside `dev` the admin binary refuses to start without it. |
...
See `gateway/cmd/all-in-one` (introduced in Phase 5). Boots an embedded
